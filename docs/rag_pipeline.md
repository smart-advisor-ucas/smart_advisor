...
Both layers converge on the same tool: `record_unknown_question(question, name, email, phone)`, which posts to Telegram via `send_fallback_telegram()` and returns `{"recorded": "ok"|"failed"}`.
